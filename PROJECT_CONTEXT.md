...
| `--color-accent` | `#197F71` | Primary CTA color ("Teal") — the only color used for clickable/actionable things. **Full color history:** `#D9761F` Burnt Saffron (Sept 2026, to echo the old Wix theme) → darkened to `#AC5E19` (same month) after white-on-saffron measured only ~3.2:1, below WCAG AA's 4.5:1 → switched to Teal per a re-uploaded `DESIGN_SYSTEM.md`'s Section 0 "hard constraints" (mandating teal, calling saffron a "drift" bug) — confirmed as an intentional pivot with ASM before implementing, since it reversed ASM's own prior saffron decision → the doc's literal `#1B8A7A` measured only ~4.23:1 with white text (doc claimed ~4.7:1) — **nudged darker to `#197F71`** (ASM confirmed) for a real ~4.85:1, same hue/saturation as the doc's value. |
| `--color-accent-hover` | `#136458` | Accent hover state (~7.0:1) |
| `--color-accent-active` | `#0E473F` | Accent active/pressed state (~10.5:1) |
